...
final class PendingConfirmationService {
    ...
    func enterPending(alarm: Alarm) async {
        ...
        if alarm.cycleType == .oneTime {
            ...
            do { try await AlarmScheduler.shared.cancelAlarm(alarm) } catch { AnalyticsService.shared.recordError(error, context: "cancel_alarm_enterPending") }
        } else {
            alarm.nextFireDate = dateCalculator.calculateNextFireDate(for: alarm, from: Date())
            do { try await AlarmScheduler.shared.cancelAlarm(alarm) } catch { AnalyticsService.shared.recordError(error, context: "cancel_alarm_enterPending") }
            do { try await AlarmScheduler.shared.scheduleAlarm(alarm) } catch { AnalyticsService.shared.recordError(error, context: "schedule_alarm_enterPending") }
        }
